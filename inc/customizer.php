 ...
 */
function jetsy_customize_register( $wp_customize ) {
...
//	This is to control the header and footer background color.
    $wp_customize->add_setting( 'theme_bg_color', array(
        'default'			=> '#ff4500',  //default color
        'transport'			=> 'postMessage',
        'type'				=> 'theme_mod',  //type of setting
        'sanitize_callback' => 'sanitize_hex_color',  //use anytime user types in code
    ));

    // Control for header and footer background color.
    $wp_customize->add_control(
        new WP_Customize_Color_Control(  //created the interactive color control
            $wp_customize,
            'theme_bg_color', //where sending changes
            array(
                'label'		=> __( 'Header / footer background color', 'jetsy'),
                'section'	=> 'colors',  //what section added to
                'settings'	=> 'theme_bg_color'  //control name
            )
        )
    );
...
//	);
    ...
    $wp_customize->add_section( 'theme_options',
        array(
            'title'			=> __( 'Theme Options', 'jetsy' ),
            'priority'		=> 95,
            'capability'	=> 'edit_theme_options',
            'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'jetsy' )
        )
    );
    ...
    // Add the controls
    $wp_customize->add_control(	'jetsy_length_control',
        array(
            'type'		=> 'radio',
            'label'		=> __( 'Index/archive displays', 'jetsy' ),
...
add_action( 'customize_register', 'jetsy_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function jetsy_customize_preview_js() {
    wp_enqueue_script( 'jetsy_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'jetsy_customize_preview_js' );
...
function jetsy_sanitize_length( $value ) {
...
if ( ! function_exists( 'jetsy_header_style' ) ) :
    /**
     * Styles the header image and text displayed on the blog.
     *
     * @see jetsy_custom_header_setup().
     */
